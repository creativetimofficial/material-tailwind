...
const Label = ({ children, color }) => {
  ...
  return (
    <>
      {showLabel ? (
        <div
          className={`flex items-center justify-between py-1 pl-3 pr-1 rounded-full bg-${color}-100 last:mr-0 mr-1 transition-all duration-300`}
        >
          <span
          ...
          <span
            className={`material-icons bg-${color}-900 text-white text-sm rounded-full p-0.5 cursor-pointer grid place-items-center`}
            onClick={closeLabel}
          >
            close
          </span>
