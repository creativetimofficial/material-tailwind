...
export default function InputPhoneDemo() {
  ...
  document.getElementById('dropdownButtonPhone').addEventListener('click', function (event) {
  ...
  document.addEventListener('click', function (event) {
    const dropdownMenu = document.getElementById('dropdownMenu');
    const isClickInside =
      document.getElementById('dropdownButtonPhone').contains(event.target) ||
  ...
  <label class="block mb-2 text-sm text-slate-800 font-semibold antialiased">
    Enter Phone Number
  </label>
  <div class="relative">
    <div class="absolute top-2 left-0 flex items-center pl-3">
      <button id="dropdownButtonPhone" class="h-full text-sm flex items-center bg-transparent text-slate-700 focus:outline-none">
      ...
      <div id="dropdownMenu" class="min-w-[150px] absolute left-0 w-full mt-10 hidden bg-white border border-slate-200 rounded-md shadow-lg z-10">
