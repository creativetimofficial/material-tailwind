...
Algolia Search uses the `@docsearch/react` library to provide the search functionality. Make sure to install it before using the example.
...
`@docsearch/react` requires the `@docsearch/css` stylesheet to be imported in your project. Make sure to import it in your project.
...
The Algolia Search uses `DocSearch` component from the `@docsearch/react` library. The `DocSearch` component requires the `appId`, `apiKey`, and `indexName` as props to work.
...
## Customising The Search Dialog (Optional)
