...
You can use icons to behave as a `Checkbox` using the `children` prop for `Checkbox`, you can use the `peer-checked` class to set the styles for the icons based on the `Checkbox` state.
...
You can customize the `Checkbox` using the  `className`, `inputClassName`, and `iconClassName` props.
