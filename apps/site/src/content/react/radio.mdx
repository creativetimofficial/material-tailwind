...
You can use icons to behave as a `Radio` using the `children` prop for `Radio`, you can use the `peer-checked` class to set the styles for the icons based on the `Radio` state.
...
You can customize the `Radio` using the `className`, `inputClassName` and `iconClassName` props.
