...
The `Checkbox` components can be used with a label to provide more context to the user. In the example below, we've added a label to the checkbox by using the `Checkbox` and `Typography` components together.
...
To make it work, you need to add `group` class to the `Checkbox` component and use `group-data-[checked=true]` for the icons to show/hide based on the checkbox state.
...
Use the example below to create a `Checkbox` with a link to the terms and conditions inside it's label.
