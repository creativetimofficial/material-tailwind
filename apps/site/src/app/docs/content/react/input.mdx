...
We provide helpful props like `isError` and `isSuccess` for showing error and success states of an input. In the example below, we've showcased the error and success states that you can use in your project.
...
The `Input` components can be used with a label to provide more context to the user. In the example below, we've added a label to the input by using the `Input` and `Typography` components together.
