...
We provides different `IconButton` variants like `ghost`, `outline-solid`, `solid`, and `gradient` so you can adapt it easily to your needs. You can simply use the `variant` prop to change the `IconButton` variant.
