...
export function toggleAccordion(event) {
  ...
  const parentElement = event.currentTarget.closest("[data-accordion-container]");
  const mode = parentElement?.getAttribute("data-accordion-mode");
  ...
  if (targetID?.startsWith("#")) {
    ...
    if (mode === "exclusive" && parentElement) {
      parentElement.querySelectorAll("[data-accordion-toggle]").forEach(button => {
        const otherTargetID = button.getAttribute("data-accordion-target");
...
export function initAccordion() {
  document.querySelectorAll("[data-accordion-toggle]").forEach(button => {
    if (!initializedAccordionElements.has(button)) {
      ...
      const targetElement = document.querySelector(button.getAttribute("data-accordion-target"));
  ...
  document.querySelectorAll("[data-accordion-container]").forEach(container => {
    ...
    const buttons = container.querySelectorAll("[data-accordion-toggle]");
