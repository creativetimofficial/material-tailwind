...
<CodeTabs>
  ...
  <TabPanel value={2}>
    ```html
    <button
      class="relative inline-block py-2.5 px-6 cursor-pointer overflow-hidden rounded-lg bg-pink text-xs font-medium uppercase"
      dialog-trigger="true"
      data-ripple-light="true"
    >
      Demo dialog
    </button>
    <div class="fixed top-0 left-0 z-0 w-full h-full overflow-x-hidden overflow-y-auto opacity-0 invisible">
      <div class="fixed top-0 left-0 w-screen h-screen z-[1200] bg-[rgba(0,0,0,0.5)] transition-[opacity .15s linear]" dialog-close="true"></div>
      <div class="flex items-center relative w-auto pointer-events-none m-[1.75rem auto] max-w-[31rem] min-h-[calc(100% - 15rem)]">
        <div class="relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md shadow-dialog outline-0 z-[1300]">
          <div class="flex shrink-0 items-center space-between p-4 border-b border-[#dee2e6]">
            <h6 class="mb-0">Dialog title</h6>
            <button
              type="button"
              class="me-0 button-close"
