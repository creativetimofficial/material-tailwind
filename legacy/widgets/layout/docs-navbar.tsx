...
import { Logo, Search } from "legacy/widgets";
import { formatNumber } from "legacy/utils";
...
function NavItem({
  ...
  return (
    <LinkEl passHref href={href ? href : ""}>
      <ListItem className="text-blue-gray-800 hover:text-primary px-4 py-2.5 text-sm">
        {children}
      </ListItem>
    </LinkEl>
  );
...
export function DocsNavbar({ slug, setMobileNav }: DocsNavbar) {
  ...
  const menuOpenIcon = (
    <svg
      ...
      className="stroke-primary text-primary h-6 w-6"
      strokeWidth={2}
    >
      <path
        strokeLinecap="round"
      ...
      <div className="ml-2 flex items-center gap-2">
        <div className="group relative">
          <Input
            type="email"
            placeholder="Search"
            className="!border-blue-gray-50 text-blue-gray-800 placeholder:text-blue-gray-600 focus:!border-primary focus:!border-t-primary group-hover:!border-primary !w-full !border-[1.5px] bg-white ring-4 ring-transparent"
            labelProps={{
              className: "hidden",
            }}
          ...
          <div className="absolute right-3.5 top-2/4 -translate-y-2/4">
            <kbd className="border-blue-gray-100 rounded border bg-white px-1 pb-0 pt-px text-xs font-medium text-gray-900 shadow shadow-black/5">
        ...
        <Tooltip content="Help with a star" placement="bottom" offset={-2.5}>
          <a
            target="_blank"
            rel="noreferrer"
            className="text-primary flex items-center gap-2 p-1.5"
          ...
          >
            <Chip
              ...
              className="flex-items-center bg-primary gap-2 py-[3px] !pr-2 text-xs"
            />
  ...
  return (
    <div className="sticky top-0 z-[999] flex w-full items-center">
      <Navbar
        className="!border-blue-gray-50 w-full max-w-full rounded-none border-b-[1.5px] bg-white py-1.5 !pl-2 !pr-3 lg:!px-4 lg:!py-0.5"
        shadow={false}
      >
          ...
          <div className={`flex w-full items-center !justify-between`}>
            ...
            >
              ...
              <Chip
                value="v2.1.0"
                variant="outlined"
                className="border-blue-gray-50 text-primary ml-2 rounded-full border-[1.5px] pb-1 pt-1.5"
              />
              <hr className="border-primary/10 mx-2 h-8 border-r" />
              <Typography
                variant="small"
            ...
            <IconButton
              variant="text"
              className="text-primary ml-auto h-6 w-6 hover:bg-transparent focus:bg-transparent active:bg-transparent lg:hidden"
              ripple={false}
              onClick={() => setOpen(!open)}
            >
              {open ? menuCloseIcon : menuOpenIcon}
            </IconButton>
            <div className="lg:base-auto lg-max:max-h-0 hidden flex-grow basis-full items-center lg:flex">
              {navbarMenu}
            </div>
          </div>
        ...
        <div className="border-blue-gray-50 mt-2 flex items-center border-t pb-2 pt-4 lg:hidden">
          <button
            type="button"
